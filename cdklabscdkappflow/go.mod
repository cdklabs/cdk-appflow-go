module github.com/cdklabs/cdk-appflow-go/cdklabscdkappflow

go 1.18

require (
	github.com/aws/jsii-runtime-go v1.93.0
	github.com/aws/aws-cdk-go/awscdkgluealpha/v2 v2.116.1-alpha.0
	github.com/aws/aws-cdk-go/awscdkredshiftalpha/v2 v2.116.1-alpha.0
	github.com/aws/aws-cdk-go/awscdk/v2 v2.116.1
	github.com/aws/constructs-go/constructs/v10 v10.0.5
	github.com/cdklabs/awscdk-asset-awscli-go/awscliv1/v2 v2.2.201 // indirect
	github.com/cdklabs/awscdk-asset-kubectl-go/kubectlv20/v2 v2.1.2 // indirect
	github.com/cdklabs/awscdk-asset-node-proxy-agent-go/nodeproxyagentv6/v2 v2.0.1 // indirect
)
